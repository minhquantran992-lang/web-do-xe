...
const createBrandAdmin = asyncHandler(async (req, res) => {
  ...
  if (legacy) {
    await Brand.updateOne(
      { _id: legacy._id },
      { $set: { vehicleType, key, logo, searchText: buildSearchText(legacy?.name, key, vehicleType, legacy?._id) } }
    );
...
const updateBrandAdmin = asyncHandler(async (req, res) => {
  ...
  if (patch.name !== undefined || patch.vehicleType !== undefined || patch.key !== undefined) {
    const nextName = patch.name ?? existing.name;
    const nextType = patch.vehicleType ?? existing.vehicleType;
    const nextKey = patch.key ?? existing.key;
    patch.searchText = buildSearchText(nextName, nextKey, nextType, id);
  }
