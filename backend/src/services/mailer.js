...
const sendEmail = async ({ to, subject, text, html }) => {
  ...
  const isAuthCodeEmail = /(otp|ma\s*xac\s*thuc|mã\s*xác\s*thực|xac\s*minh|xác\s*minh)/i.test(subjectText);
  const from =
    isAuthCodeEmail
      ? process.env.EMAIL_FROM_OTP || process.env.EMAIL_FROM || 'eloride <no-reply@carbanana.local>'
      : process.env.EMAIL_FROM || 'no-reply@carbanana.local';
