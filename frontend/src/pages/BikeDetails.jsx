      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold">
            {[bike.brand, displayName].filter(Boolean).join(' ')}
          </h1>
        </div>
        <Link to="/bikes" className="text-sm text-sky-300 hover:text-sky-200">
          {t('bike_details_back')}
        </Link>
      </div>
        ...
        <div className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onCustomize}
              className="inline-flex flex-1 items-center justify-center rounded-xl bg-sky-400 px-4 py-3 text-sm font-medium text-zinc-950 transition hover:bg-sky-300"
            >
              {t('carcard_customize')}
            </button>
            <Link
