...
const Home = () => {
  ...
  const styleChoices = useMemo(
    () => [
      { key: 'racing', title: t('landing_style_racing_title'), desc: t('landing_style_racing_desc'), accent: 'sky' },
      { key: 'kieng', title: t('landing_style_kieng_title'), desc: t('landing_style_kieng_desc'), accent: 'cyan' },
      { key: 'touring', title: t('landing_style_touring_title'), desc: t('landing_style_touring_desc'), accent: 'amber' }
    ],
    [t]
  );
    ...
    <div className="-mx-4 -my-6 space-y-16 bg-black">
      <section className="relative overflow-hidden border-b border-zinc-800 bg-zinc-950 px-4 py-16 sm:py-20">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -left-24 -top-24 h-[28rem] w-[28rem] rounded-full bg-sky-500/14 blur-[110px]" />
        ...
        <div className="relative mx-auto grid w-full max-w-7xl items-center gap-10 lg:grid-cols-[1.15fr_.85fr]">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/25 bg-sky-500/10 px-3 py-1 text-xs font-semibold text-sky-200">
              <span className="h-1.5 w-1.5 rounded-full bg-sky-400 shadow-[0_0_12px_rgba(14,165,233,0.65)]" />
              {t('landing_badge')}
            </div>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <button
                ...
                className="group inline-flex items-center gap-2 rounded-xl bg-sky-400 px-6 py-3 text-sm font-bold text-zinc-950 transition hover:bg-sky-300 hover:shadow-[0_0_24px_rgba(14,165,233,0.35)]"
              >
            ...
            <form onSubmit={onSearchSubmit} className="mt-8">
              <div ref={searchWrapRef} className="relative max-w-2xl">
                <div className="relative">
                  <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-r from-sky-500/25 to-cyan-500/25 opacity-70 blur" />
                ...
                {searchOpen && query.trim() ? (
                  <div className="absolute left-0 right-0 top-[calc(100%+10px)] z-30 overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/85 shadow-2xl backdrop-blur">
                      ...
                      <div className="divide-y divide-white/5">
                        {suggestions.map((s) => (
                          ...
                          >
                            ...
                            <div className="rounded-full bg-sky-500/10 px-3 py-1 text-xs font-semibold text-sky-200">
                              {t('landing_go')}
                            </div>
                          </button>
                        ))}
                    ...
                    <div className="flex items-center justify-between border-t border-white/5 bg-white/3 px-4 py-2 text-xs text-zinc-400">
                      ...
                      <Link to="/bikes" className="font-semibold text-sky-300 hover:text-sky-200">
                        {t('hero_browse_all')} →
                      </Link>
                    </div>
                  </div>
          ...
          <div className="relative">
            <div className="absolute -inset-2 rounded-3xl bg-gradient-to-b from-sky-500/15 via-cyan-500/10 to-transparent blur-xl" />
                ...
                <div className="absolute bottom-4 left-4 right-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-white/10 bg-black/45 p-4 backdrop-blur">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <span className="h-1.5 w-1.5 rounded-full bg-sky-400 shadow-[0_0_10px_rgba(14,165,233,0.6)]" />
      ...
      <section id="guided-builder" className="mx-auto w-full max-w-7xl px-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-xs font-bold tracking-wider text-sky-300/90">{t('landing_guided_badge')}</div>
          ...
          <Link to="/bikes" className="inline-flex items-center gap-2 text-sm font-bold text-sky-300 hover:text-sky-200">
        ...
        <div className="mt-6 rounded-3xl border border-white/10 bg-zinc-950/60 p-5 backdrop-blur sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs font-bold text-zinc-300">
              <span className={`grid h-7 w-7 place-items-center rounded-full border ${builderStep >= 1 ? 'border-sky-400/50 bg-sky-500/15 text-sky-200' : 'border-white/10 bg-white/5 text-zinc-400'}`}>1</span>
              ...
              <span className="text-zinc-600">→</span>
              <span className={`grid h-7 w-7 place-items-center rounded-full border ${builderStep >= 3 ? 'border-amber-300/50 bg-amber-400/12 text-amber-200' : 'border-white/10 bg-white/5 text-zinc-400'}`}>3</span>
          ...
          <div className="mt-4">
            <div className="h-2 w-full overflow-hidden rounded-full bg-white/7">
              <div
                className="h-full rounded-full bg-gradient-to-r from-sky-400 via-cyan-300 to-amber-300 transition-[width] duration-500"
                style={{ width: `${builderProgress}%` }}
              />
            </div>
            <div className="mt-2 flex items-center justify-between text-[11px] font-semibold text-zinc-500">
              <span>{t('common_step')} {builderStep}/3</span>
                    ...
                    {vehicleChoices.map((c) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group relative overflow-hidden rounded-2xl border text-left transition ${active ? 'border-sky-400/50 bg-sky-500/10' : 'border-white/10 bg-white/5 hover:bg-white/8 hover:border-white/20'}`}
                        >
                          <div className="absolute inset-0 opacity-0 transition duration-300 group-hover:opacity-100">
                            <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-sky-500/14 blur-2xl" />
                          </div>
                          <div className="relative flex items-center gap-4 p-4">
                            <div className="h-14 w-20 shrink-0 overflow-hidden rounded-xl bg-black/40">
                              {c?.image ? (
                                <img
                                  src={resolveAssetUrl(String(c.image))}
                                  alt=""
                            ...
                            <div className={`rounded-full px-3 py-1 text-xs font-bold ${active ? 'bg-sky-500/15 text-sky-200' : 'bg-white/5 text-zinc-200'}`}>
                              {t('landing_select')}
                            </div>
                          </div>
                        </button>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    {styleChoices.map((s) => {
                      ...
                      const accent =
                        s.accent === 'sky'
                          ? 'hover:border-sky-400/50 hover:bg-sky-500/8'
                          : s.accent === 'cyan'
                            ? 'hover:border-cyan-400/50 hover:bg-cyan-500/8'
                            : 'hover:border-amber-300/50 hover:bg-amber-400/8';
                      const activeCls =
                        s.accent === 'sky'
                      ...
                      return (
                        ...
                        >
                          <div className="absolute inset-0 opacity-0 transition duration-300 group-hover:opacity-100">
                            <div className={`absolute -right-8 -top-8 h-32 w-32 rounded-full ${s.accent === 'sky' ? 'bg-sky-500/16' : s.accent === 'cyan' ? 'bg-cyan-500/16' : 'bg-amber-400/14'} blur-2xl`} />
                          </div>
                          <div className="relative">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl bg-sky-400 px-5 py-3 text-sm font-black text-zinc-950 transition hover:bg-sky-300 disabled:cursor-not-allowed disabled:opacity-50"
                      >
      ...
      <section className="mx-auto w-full max-w-7xl px-4">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link to="/builds" className="hidden items-center gap-2 text-sm font-bold text-sky-300 hover:text-sky-200 sm:inline-flex">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {popularBuilds.map((b) => {
            ...
            return (
              ...
              >
                <div className="absolute inset-0 opacity-0 transition duration-300 group-hover:opacity-100">
                  <div className="absolute -right-10 -top-10 h-48 w-48 rounded-full bg-sky-500/12 blur-3xl" />
                </div>
                <div className="relative aspect-[16/11] bg-black">
                ...
                <div className="relative space-y-3 p-5">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="inline-flex items-center gap-2 rounded-full bg-sky-500/12 px-3 py-1 text-xs font-black text-sky-200">
                      {t('landing_remix')}
                      <span className="transition-transform group-hover:translate-x-0.5">→</span>
                    </div>
                  </div>
