...
const CarCard = ({ car }) => {
  ...
  const rawImage = car?.thumbnailUrl || car?.imageUrl || car?.image || '';
  const imageSrc = rawImage ? resolveAssetUrl(String(rawImage)) : PLACEHOLDER_IMAGE;
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900/40">
      ...
      <div className="space-y-3 p-4">
        <div className="text-base font-semibold">{displayCarName(car.name)}</div>
        <button
          type="button"
          disabled={!canCustomize}
          onClick={onCustomize}
          className="inline-flex items-center justify-center rounded bg-sky-400 px-3 py-2 text-sm font-medium text-zinc-950 hover:bg-sky-300"
        >
          {t('carcard_customize')}
        </button>
      </div>
